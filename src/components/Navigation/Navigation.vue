    ...
    <div class="in">
      <ul class="nav-list">
        ...
        <li class="nav-li" @click="changeActive($event,1)">
          <router-link class="nav-li-link" :class="{'active':active==1}" to="/home"><i class="fa fa-home fa-fw"></i> 首页介绍</router-link>
        </li>
        ...
        <li class="nav-li" @click="changeActive($event,4)">
          <router-link class="nav-li-link" :class="{'active':active==4}" to="/services"><i class="fa fa-folder fa-fw"></i> 个人简历</router-link>
        </li>
...
export default {
  ...
  methods: {
    ...
    ctrlCanvas () {
      if (!this.cShow) {
        this.$confirm('确定要开启 canvas 背景效果吗? canvas会使用手机GPU加速，会引起手机发烫和加快耗电')
          .then(() => {
            Bus.$emit('ctrlCanvas')
            Bus.$emit('closeMeun')
            this.show = false
